...
import { HeroIcon } from "@/components/marketing/hero-icon-registry";
import type { Tables } from "@/lib/supabase/database.types";

const INTERVALLE_MS = 5000;

export function HeroCarousel({ slides }: { slides: Tables<"hero_slides">[] }) {
  ...
  return (
    ...
    >
      ...
      <p className="sr-only" aria-live="polite">
        {slide.categorie} — image {index + 1} sur {slides.length}
      </p>
          ...
          >
            {s.image_url ? (
              ...
              <div className="flex h-full w-full flex-col items-center justify-center gap-3 bg-el-gradient">
                <HeroIcon slug={s.icon_slug} className="size-12 text-white/90" />
